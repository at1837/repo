...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for s in self.mdp.getStates():
            predecessor = set()
            for s_next in self.mdp.getStates():
                for action in self.mdp.getPossibleActions(s_next):
                    for next_state, t in self.mdp.getTransitionStatesAndProbs(s_next, action):
                        if t != 0 and next_state == s:
                            predecessor.add(s_next)
            all_predecessors[s] = predecessor

        fringe = util.PriorityQueue()
        for s in self.mdp.getStates():
            if not self.mdp.isTerminal(s):
                curr_value = self.values[s]
                q_values = [self.getQValue(s, action) for action in self.mdp.getPossibleActions(s)]
                highest_q_values = max(q_values)
                diff = abs(curr_value - highest_q_values)
                fringe.push(s, -diff)
        ...
        for i in range(self.iterations):
            ...
            if not self.mdp.isTerminal(s):
                q_values = [self.getQValue(s, action) for action in self.mdp.getPossibleActions(s)]
                highest_q_values = max(q_values)
                self.values[s] = highest_q_values

                for p in all_predecessors[s]:
