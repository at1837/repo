...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        relu = nn.ReLU(nn.AddBias(xw1, self.b1))
    ...
    def train(self, dataset):
        ...
        while check < 0.98:
            for x, y in dataset.iterate_once(batch_size):
                ...
                self.w1.update(grad_wrt_w1, -0.2)
                self.w2.update(grad_wrt_w2, -0.2)
                self.w3.update(grad_wrt_w3, -0.2)
                self.b1.update(grad_wrt_b1, -0.2)
                self.b2.update(grad_wrt_b2, -0.2)
                self.b3.update(grad_wrt_b3, -0.2)
            offen += 1
            if offen == 5:
                offen = 0
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        # Initialize your model parameters here
        "*** YOUR CODE HERE ***"
        self.wi = nn.Parameter(self.num_chars, 50)
        self.bi = nn.Parameter(1, 50)
        self.x = nn.Parameter(self.num_chars, 50)
        self.h = nn.Parameter(50, 50)
        self.b = nn.Parameter(1, 50)
        self.wf = nn.Parameter(50, len(self.languages))
    ...
    def run(self, xs):
        ...
        """
        "*** YOUR CODE HERE ***"
        z = nn.ReLU(nn.AddBias(nn.Linear(xs[0], self.wi), self.bi))
        for char in xs[1:]:
            z = nn.ReLU(nn.AddBias(nn.Add(nn.Linear(char, self.x),\
                          nn.Linear(z, self.h)), self.b))
    ...
    def train(self, dataset):
        ...
        while check < .86:
            for x, y in dataset.iterate_once(batch_size):
                loss = self.get_loss(x, y)
                grad_wrt_wi, grad_wrt_bi, grad_wrt_x, grad_wrt_h, grad_wrt_b, grad_wrt_wf, grad_wrt_bf = nn.gradients(loss, [self.wi, self.bi, self.x, self.h, self.b, self.wf, self.bf])
                self.wi.update(grad_wrt_wi, -0.2)
                self.bi.update(grad_wrt_bi, -0.2)
                self.x.update(grad_wrt_x, -0.2)
                self.h.update(grad_wrt_h, -0.2)
                self.b.update(grad_wrt_b, -0.2)
                self.wf.update(grad_wrt_wf, -0.2)
                self.bf.update(grad_wrt_bf, -0.2)
            offen += 1
            if offen == 5:
                offen = 0
                check = dataset.get_validation_accuracy()
