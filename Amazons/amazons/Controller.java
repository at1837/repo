...
final class Controller {
    ...
    void play() {
        ...
        _winner = null;
        _board.init();
        _board.getManual().add(Piece.WHITE);
        _board.getAuto().add(Piece.BLACK);
        ...
        while (_playing) {
            ...
            if (_winner == null) {
                ...
                if (_manualPlayerTemplate instanceof GUIPlayer) {
                    JOptionPane.showMessageDialog(null,
                                _winner.toName() + " Win!", "Amazon Message",
                                0);
                }
    ...
    private Command[] _commands = {
        ...
        new Command("(manual+ +(white)?+(black)?)$", this::doManual),
        new Command("(auto+ +(white)?+(black)?)$", this::doAuto),
    };
    ...
    private void doNew(Matcher unused) {
        List<Piece> auto = new ArrayList<>(_board.getAuto());
        List<Piece> manual = new ArrayList<>(_board.getManual());
        _board.init();
        for (Piece p: auto) {
            _board.getAuto().add(p);
        }
        for (Piece p: manual) {
            _board.getManual().add(p);
        }
        _winner = null;
    }
    ...
    /** Command "manual C" with input M. */
    private void doManual(Matcher m) {
        if (m.group(0).equals("manual white")) {
            _white = _manualPlayerTemplate.create(WHITE, this);
            _board.getAuto().remove(Piece.WHITE);
            if (!_board.getManual().contains(Piece.WHITE)) {
                _board.getManual().add(Piece.WHITE);
            }
        } else {
            _black = _manualPlayerTemplate.create(BLACK, this);
            _board.getAuto().remove(Piece.BLACK);
            if (!_board.getManual().contains(Piece.BLACK)) {
                _board.getManual().add(Piece.BLACK);
            }
        }
    }

    /** Command "auto C" with input M. */
    private void doAuto(Matcher m) {
        if (m.group(0).equals("auto white")) {
            _white = _autoPlayerTemplate.create(WHITE, this);
            _board.getManual().remove(Piece.WHITE);
            if (!_board.getAuto().contains(Piece.WHITE)) {
                _board.getAuto().add(Piece.WHITE);
            }
        } else {
            _black = _autoPlayerTemplate.create(BLACK, this);
            _board.getManual().remove(Piece.BLACK);
            if (!_board.getAuto().contains(Piece.BLACK)) {
                _board.getAuto().add(Piece.BLACK);
            }
        }
    }
    ...
    /** Command "Move notation" with input M. */
    private void doMove(Matcher m) {
        Move move = Move.mv(m.group(0));
        _board.makeMove(move);
        _winner = _board.winner();
        if (_winner != null) {
            if (_winner.toSymbol().equals("W")) {
